...
export const Navbar = () => {
  ...
  return (
    <>
      <header className="bg-white border-b border-gray-100 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <NavLink to="/dashboard" className="flex items-center gap-3 no-underline min-w-0">
              <div className="w-10 h-10 rounded-xl bg-white border border-gray-200 flex items-center justify-center overflow-hidden shadow-sm flex-shrink-0">
                <img
              ...
              <span className="text-lg sm:text-xl font-bold text-gray-900 truncate">UniPath</span>
            </NavLink>
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <button
                ...
                aria-haspopup
                className="flex items-center gap-2 sm:gap-3 p-1.5 sm:pr-3 rounded-full hover:bg-gray-50 transition-all border border-transparent hover:border-gray-200 group bg-transparent cursor-pointer"
              >
                ...
                <i className="hidden sm:inline pi pi-chevron-down text-[10px] text-gray-400 group-hover:text-blue-600 transition-colors ml-1" />
              </button>
            </div>
          </div>
        </div>
      ...
      <nav className="mobile-tabbar md:hidden fixed inset-x-0 bottom-0 z-50 border-t border-gray-200 bg-white/95 backdrop-blur-md">
        <div className="h-full px-2 pb-1 grid grid-cols-4 gap-1 items-center">
          {mobileTabLinks.map((link) => (
            <NavLink
              key={link.path}
              to={link.path}
              className={({ isActive }) =>
                `h-11 rounded-xl flex flex-col items-center justify-center gap-1 text-[10px] font-semibold transition-colors no-underline ${
                  isActive
